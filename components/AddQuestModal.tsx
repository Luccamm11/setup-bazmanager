  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-primary rounded-lg p-6 w-full max-w-lg relative border border-border-color" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold mb-6 text-text-primary">{t('common:forms.add_personal_quest')}</h2>
        
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="title" className="block text-sm font-medium text-text-secondary mb-1">{t('common:forms.title')}</label>
            <div className="relative">
                <input type="text" id="title" value={title} onChange={e => setTitle(e.target.value)} required className="block w-full bg-background border border-border-color rounded-md py-2 px-3 text-white focus:outline-none focus:ring-accent-primary focus:border-accent-primary sm:text-sm pr-10" placeholder={t('common:forms.placeholder_title')} />
                <AiTextGenerator
                    apiKey={apiKey}
          ...
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-text-secondary mb-1">{t('common:forms.description')}</label>
            <div className="relative">
                <textarea id="description" value={description} onChange={e => setDescription(e.target.value)} rows={3} className="block w-full bg-background border border-border-color rounded-md py-2 px-3 text-white focus:outline-none focus:ring-accent-primary focus:border-accent-primary sm:text-sm pr-10" placeholder={t('common:forms.placeholder_description')} />
                 <AiTextGenerator
                    apiKey={apiKey}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="realm" className="block text-sm font-medium text-text-secondary">{t('common:forms.realm')}</label>
            ...
            <div>
              <label htmlFor="difficulty" className="block text-sm font-medium text-text-secondary">{t('common:forms.difficulty')}</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             <div>
                <label htmlFor="xp" className="block text-sm font-medium text-text-secondary">{t('common:forms.xp_reward')}</label>
             ...
             <div>
                <label htmlFor="credits" className="block text-sm font-medium text-text-secondary">{t('common:forms.credits_reward')}</label>
            ...
            <div>
                <label htmlFor="duration" className="block text-sm font-medium text-text-secondary">{t('common:forms.estimated_time')}</label>
