    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center py-20 animate-pulse">
                ...
                <p className="text-text-secondary font-bold">{t('analytics:team.status_loading')}</p>
            </div>
        );
    }
    ...
    if (!members || members.length === 0) {
        return (
            <div className="text-center py-20 bg-primary/20 rounded-3xl border border-dashed border-white/10">
                <p className="text-text-secondary font-medium">{t('analytics:team.no_data')}</p>
                <p className="text-xs text-text-muted mt-2">{t('analytics:team.no_data_hint')}</p>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <KpiCard icon={<Users size={24} />} title={t('analytics:team.kpi_total_members')} value={members.length} color="text-accent-primary" />
                <KpiCard icon={<Star size={24} />} title={t('analytics:team.kpi_total_xp')} value={totalXp.toLocaleString()} color="text-accent-secondary" />
                <KpiCard icon={<Award size={24} />} title={t('analytics:team.kpi_avg_level')} value={avgLevel} color="text-accent-tertiary" />
                <KpiCard icon={<Zap size={24} />} title={t('analytics:team.kpi_quests_done')} value={totalQuests} color="text-accent-red" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                ...
                <motion.div variants={itemVariants} className="lg:col-span-2 bg-primary/40 backdrop-blur-2xl p-5 sm:p-8 rounded-3xl border border-white/5 shadow-glass relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-8 relative z-10">
                        ...
                        <h3 className="text-xl sm:text-2xl font-black text-text-primary tracking-tight">{t('analytics:team.ranking_title')}</h3>
                    </div>
                            ...
                            <div key={member.username} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-white/10 hover:bg-white/10 transition-all group">
                                ...
                                <div className="text-right flex items-center gap-6">
                                    <div className="hidden sm:block">
                                        <p className="text-[10px] text-text-muted uppercase font-black tracking-widest leading-none mb-1">{t('analytics:team.member_status')}</p>
                                        <div className="flex items-center gap-1.5 justify-end">
                                            ...
                                            <span className="text-[10px] font-bold text-text-secondary">{t('analytics:team.member_streak', { count: member.streak })}</span>
                                        </div>
                                    </div>
                                    <div className="min-w-[80px]">
                                        <p className="text-sm font-black text-white">{member.xpTotal.toLocaleString()}</p>
                                        <p className="text-[10px] text-accent-secondary uppercase font-bold tracking-wider">{t('analytics:team.member_xp_label')}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div className="space-y-6 sm:space-y-8">
                    <motion.div variants={itemVariants} className="bg-gradient-to-br from-accent-secondary/20 to-accent-primary/10 p-8 rounded-3xl border border-accent-secondary/20 shadow-glow-secondary relative overflow-hidden group">
                        ...
                        <h4 className="text-[10px] font-black text-accent-secondary uppercase tracking-[0.3em] mb-2 leading-none">{t('analytics:team.mvp_label')}</h4>
                        ...
                        <div className="space-y-3 relative z-10">
                             <div className="flex justify-between text-[11px] font-black uppercase text-text-secondary tracking-wider">
                                <span>{t('analytics:team.collective_impact')}</span>
                             ...
                             <p className="text-[11px] text-text-muted font-medium italic leading-relaxed pt-2">
                                {t('analytics:team.mvp_description')}
                             </p>
                        </div>
                    </motion.div>
                    ...
                    <motion.div variants={itemVariants} className="bg-primary/40 backdrop-blur-2xl p-6 sm:p-8 rounded-3xl border border-white/5 shadow-glass relative overflow-hidden">
                        ...
                        <h4 className="text-lg sm:text-xl font-black text-text-primary tracking-tight mb-6">{t('analytics:team.management_goals')}</h4>
                        <div className="space-y-4 relative z-10">
                             <div className="p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-accent-tertiary/30 transition-all cursor-default group">
                                <div className="flex items-center gap-2 mb-1.5">
                                    ...
                                    <p className="text-[10px] text-text-muted font-black uppercase tracking-widest">{t('analytics:team.rank_evolution')}</p>
                                </div>
                                <p className="text-sm font-bold text-text-secondary group-hover:text-white transition-colors">{t('analytics:team.rank_evolution_goal')}</p>
                             </div>
                             <div className="p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-accent-primary/30 transition-all cursor-default group">
                                <div className="flex items-center gap-2 mb-1.5">
                                    ...
                                    <p className="text-[10px] text-text-muted font-black uppercase tracking-widest">{t('analytics:team.bleed_pace')}</p>
                                </div>
                                <p className="text-sm font-bold text-text-secondary group-hover:text-white transition-colors">{t('analytics:team.bleed_pace_goal')}</p>
                             </div>
                        </div>
                    </motion.div>
                </div>
