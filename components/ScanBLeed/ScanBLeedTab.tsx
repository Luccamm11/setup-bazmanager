        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="bg-[#16192a] border border-white/10 rounded-2xl p-6 space-y-5 shadow-2xl">
              ...
              {scanError && (
                <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  {imageSrc && (
                    <button
                      type="button"
                      disabled={isScanning}
                      onClick={() => processImageWithGemini(imageSrc)}
                      className="px-3 py-1.5 rounded-lg bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 border border-amber-500/30 text-[11px] font-bold shrink-0 flex items-center gap-1.5 transition-all"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isScanning ? 'animate-spin' : ''}`} /> Tentar Novamente com IA
                    </button>
                  )}
                </div>
              )}
